        ...
        <div class="col-span-1 bg-gray rounded p-3 border">
          ...
          >
            {(s: any) => {
              ...
              const cpuMilli =
                typeof s.cpuMilli === 'number' ? s.cpuMilli : undefined
              const memoryMB =
                typeof s.memoryMB === 'number' ? s.memoryMB : undefined
              const storageMB =
                typeof s.storageMB === 'number' ? s.storageMB : undefined
              const vramMB = typeof s.vramMB === 'number' ? s.vramMB : undefined
              const supportsCluster = !!s.supportsCluster
              const online =
                (lastTs != null
                  ? Date.now() - lastTs < 5 * 60 * 1000
                  : false) || String(s.state).toLowerCase() === 'online'
